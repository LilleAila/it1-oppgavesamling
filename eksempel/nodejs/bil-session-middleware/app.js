// Video med forklaring: https://youtu.be/Qbqd2jFspVI (NB: elendig lydkvalitet)
/*
...
app.post("/leggtilbil", (req, res) => {
    const { registreringsnummer, merke, modell } = req.body;
    const personnummer = req.session.bruker.id;

    if (!registreringsnummer || !merke || !modell) {
        return res.status(400).json({ message: "Mangler data i forespørselen." });
    }
...
// Rute for å hente alle postnummer (SQL)
